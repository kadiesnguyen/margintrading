...
module.exports = {
    getUserDepositHis: (username, callback) => {
        let sql = `select * from trade_history where email = ? AND delete_status = 0 and type_key = "nt" ORDER BY id DESC LIMIT 20`
        db.query(
            sql,
    ...
    getUserWitdrawalHis: (username, callback) => {
        let sql = `select * from trade_history where email = ? AND delete_status = 0 and type_key = "rt" ORDER BY id DESC LIMIT 20`
        db.query(
            sql,
            [username], (error, results, fields) => {
                if (error) {
    ...
    getAllDepositHis: (user, callback) => {
       if(user.x == 1){
        db.query(
       ...
       if(user.x == 2){
        db.query(
            `select * from trade_history where 
                email in (select username from users where upline_id = ?)
                and delete_status = 0 and status = 1 and (type_key = "nt" or type_key = "nn") order by id desc `,
    ...
    getAllWithDrawalHis: (user, callback) => {
        if(user.x == 1){
            db.query(
        ...
        if(user.x == 2){
            db.query(
                `select * from trade_history where 
                    email in (select username from users where upline_id = ?) 
                    and delete_status = 0 and status = 1 and type_key = "rt" order by id desc `,
                [user.ref_code], (error, results, fields) => {
                    if(error){
                        return callback(error);
                     }
