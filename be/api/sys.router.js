...
app.get("/getRateCommission", (req, res) => {
	if (req.query.file) {
		const dataSys = Helper.getConfigData(req.query.file);
...
app.post("/saveRateCommission", (req, res) => {
	const fileConfig = req.body.file || `${config.PATH_SYS_COMMISSION}.json`;
    const dataSys = req.body.data || JSON.stringify(req.body);
	Helper.setConfigData(fileConfig, dataSys);
