  ...
  <div id="wallet">
    <div class="">
        <div class="w-full h-48 subBanner">
            <div class="max-w-5xl mx-auto px-4 md:px-12 h-full flex items-center info">
                <div class="flex flex-col justify-center relative">
                    <p class="text-lg color-white">
                        Tổng tài sản
                    </p>
                    <p>
                        <span class="price mr-2">
                            {{ formatPrice(balanceForUser, 0) }}
                        </span>
                    </p>
                </div>
            </div>
        ...
        <div class="w-full">
            <div class="max-w-5xl mx-auto md:px-12 px-4">
                <div class="showV mt-8" :class="{block: showVC}">
                    <div class="box-coin-wrapper">
                        ...
                        <div class="vx-row">
                            <div class="vx-col w-full lg:w-1/3 mb-3">
                                <div class="boxCoin">
                                    <div class="boxCoin-body">
                                        <div class="leftBox flex flex-col sm:flex-row">
                                            <div class="flex items-center">
                                                <span class="currency-icon VND"></span>
                                                <span class="uppercase font-bold block">Ngân hàng</span>
                                            </div>
                                            <!-- <div class="flex flex-col">
                                                <span class="uppercase font-bold">Ngân hàng</span>
                                                 <span class="capitalize colorGray">Tether</span>
                                            </div> -->
                                        </div>
                                        <div class="rightBox">
                                            <div class="flex flex-col items-end">
                                                <span class="font-bold">{{ formatPrice(balanceForUser, 0) }}</span>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <ul class="nav nav-tabs">
                                                ...
                                                <li class="item" v-else :key="indextr" v-for="(tr, indextr) in dataHisWallet">
                                                    <div class="flex" style="padding: 0 1.5rem;" @click="popupBill(tr)">
                                                        ...
                                                        <div class="block-col amount">
                                                            <!-- <div v-if="blObj.email.toUpperCase() == tr.email.toUpperCase()">
                                                                <span class="red" v-if="tr.type_key == 'rt' || tr.type_key == 'ct' || tr.type_key == 'ctsa' || tr.type_key == 'nn' || tr.type_key == 'mv'">-{{ nFormatter(tr.amount, 2) }}</span>
                                                                <span class="green" v-else>+{{ nFormatter(tr.amount, 2) }}</span>
                                                            </div> -->
                                                            <div>
                                                                <span class="green">+{{ nFormatter(tr.amount, 2) }}</span>
                                                            </div>
                                                        </div>
                                                        <div class="block-col type">
                                                        ...
                                                        <div class="block-col" style="flex: 2 1 0%;">
                                                            <div class="text-left">
                                                                <div class="item-txid-desc">
                                                                    {{ tr.type_key == 'ctsa' || tr.type_key == 'ctas' ? '' : tr.from_u + ':' }} {{ tr.type }}
                                                                </div>
                                                                <div class="item-txid-desc" v-html="getBankContent(tr.type_key, tr.currency , tr.bank)">
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="block-col note">
                                                ...
                                                <vs-pagination v-if="totalRUSDT > 0" class="mt-4 mb-2 custom-pagination" :total="totalRUSDT" v-model="currentxUSDT" @input="clickPageUSDT"></vs-pagination>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
...
import DepositSocket from "@/services/DepositSocket";
...
components: {
  ...
  data () {
    return {
        ...
        blObj: getData,
        textWalletHtml: '<span class="mr-1 color-blue uppercase green">VNĐ</span> <span>Wallet</span>',
  ...
  computed: {
    ...
    //   },
        balanceForUser(){
            return getData.balance + getData.blLive;
        }
  },
  methods: {
    ...
    popupBill(tr){
        ...
        this.descriptShow = this.descriptShow + this.typeShow;
    ...
    getSysWallet(){
        ...
        .then((res) => {
            ...
            getSetSys.quotePriceVND = this.replaceAll(this.formatPrice(g.qVND, 2), ',', '') //  giá sấp sĩ USD
  ...
  async created() {
    ...
    //   this.amountAccLive = getData.blLive;
        ...
        // DepositSocket.socket.onopen = function() {
        //     //console.log("Successfully connected to the echo websocket server...")
        //     this.onWindowLoad();
        // }.bind(this);

        DepositSocket.socket.onmessage = function(event) {
...
.subBanner{
    ...
    height: 160px;
    width: 100%;
    background: linear-gradient(180deg, #2b70fa 0%, #38bdf8 100%);
