    ...
    <div id="loading-corners" class="vs-con-loading__container">
      <vs-table ref="table" pagination :max-items="itemsPerPage" search :data="products">
        <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
          ...
          <template slot="thead">
              <vs-th sort-key="description">{{ $t('mota') }}</vs-th>
              <vs-th sort-key="buy_sell">BUY / SELL</vs-th>
              <vs-th sort-key="amount">{{ $t('sotiencuoc') }}</vs-th>
              <vs-th sort-key="win_lose">{{ $t('thangthua') }}</vs-th>
              <vs-th sort-key="result_bet">{{ $t('ketqua') }}</vs-th>
              <vs-th sort-key="open_close">{{ $t('phien') }}</vs-th>
          ...
          <template slot-scope="{data}">
            <tbody>
              <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                  <vs-td>
                      <p class="bet-description font-medium truncate">{{ tr.email }}</p>
                  </vs-td>
                  <vs-td>
                      <p class="bet-buy_sell" v-if="tr.buy_sell == 'buy'">
                          <span class="text-success">BUY</span>
                      </p>
                      <p class="bet-buy_sell" v-else><span class="text-danger">SELL</span></p>
                  </vs-td>

                  <vs-td>
                      <p class="bet-amount">
                          <!-- <IconCrypto style="width: 20px" coinname="USD" color="color" format="svg" />  -->
                          <font>{{ formatPrice(tr.amount_bet) }}</font>
                      </p>
                  </vs-td>

                  <vs-td>
                      <p class="bet-win_lose">
                      <span>{{  `${tr.amount_win != 0 ? '+'+formatPrice(tr.amount_win) : '-'+ formatPrice(tr.amount_lose)}` }}</span>
                      </p>
                  </vs-td>
                  ...
                  <vs-td>
                      ...
                      <p class="bet-result_bet" v-else><span class="text-danger">LOSE</span></p>
                  </vs-td>
