<template>
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    ...
    <VuePerfectScrollbar style="height: calc(var(--vh, 1vh) * 100 - 16px - 45px - 10px);" class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      ...
      <div class="p-6">
        <div>
          <h5>Thông tin rút tiền <b>{{ currentUser.from_u }}</b></h5>
          <div>---</div>
          <div><b>Số điện thoại:</b> {{ currentUser.email }}</div>
          <div><b>Số tiền:</b> {{ nFormatter(currentUser.amount, 2) }}</div>
          <div><b>Tên ngân hàng:</b> {{ currentUser.tenNganHang }}</div>
          <div><b>Chi nhánh:</b> {{ currentUser.chiNhanhNganHang }}</div>
          <div><b>Số tài khoản:</b> {{ currentUser.soTaiKhoan }}</div>
          <div><b>Chủ tài khoản:</b> {{ currentUser.chuTaiKhoan }}</div>
          <div><b>Thời gian yêu cầu rút tiền:</b> {{ formatDate(currentUser.created_at) }}</div>
        </div>
        
        <div>---</div>
        <vs-input label="Lý do từ chối" v-model="currentUser.note" class="mt-5 mb-5 w-full" type="text" name="item-email" />
...
export default {
  ...
  watch: {
    isSidebarActive(val) {
      ...
      this.currentUser.tenNganHang = bankArr[0];
      this.currentUser.chiNhanhNganHang = bankArr[1];
      this.currentUser.soTaiKhoan = bankArr[2];
      this.currentUser.chuTaiKhoan = bankArr[3];
    }
  },
  computed: {
  ...
  methods: {
    ...
    submitDataUpdateMoney(status){
      ...
      this.currentUser.status = status;
        ...
        DepositService.handleMoneyRut(obj)
        .then((resp) => {
          this.isSidebarActiveLocal = false;
          if(resp.data.success == 1){

            return this.$vs.notify({
                text: `${status === 1 ? 'Chấp nhận': 'Từ chối'} rút tiền ${this.currentDataMoney.from_u} thành công!`,
                color:'success',
                iconPack: 'feather',
                icon:'icon-check'});
            }else{
