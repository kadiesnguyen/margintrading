            ...
            <tbody>
              <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                ...
                <vs-td class="whitespace-no-wrap text-center hover:text-success">
                  <vx-tooltip v-if="tr.verified == 0 || tr.verified == 2" :title="tr.nick_name" color="warning" text="Từ chối xác minh">
                    <div @click.stop="doneVerify(tr.id, -1, tr)">
                          <feather-icon icon="XIcon" svgClasses="w-5 h-5 stroke-current" /> Từ chối xác minh
                    </div>
                  </vx-tooltip>
                  ...
                  <vx-tooltip v-else-if="tr.verified == 1" :title="tr.nick_name" color="danger" text="Hủy xác minh">
                    <div @click.stop="doneVerify(tr.id, 0, tr)">
